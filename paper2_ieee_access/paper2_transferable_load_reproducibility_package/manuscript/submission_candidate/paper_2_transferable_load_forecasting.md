...
![Figure 14. Local target-domain load curve used as a small adaptation case.](figures/paper2_fig2_load_curve.png)

![Figure 15. Pilot RMSE comparison on the local Hunan user-load case.](figures/paper2_fig3_load_rmse.png)

The current local user-load curve is small and is not used as the sole evidence for a transferable-learning paper. It is still useful as a target-domain feasibility case. A preliminary holdout test on the Hunan University load curve gives the following baseline results.

|Data|Model|MAE|RMSE|sMAPE|N|
|---|---|---|---|---|---|
|Hunan load|Persist-1h|0.114|0.153|3.510|78|
|Hunan load|Seasonal-24h|0.075|0.111|2.310|78|
|Hunan load|Linear-lag-cal|0.074|0.096|2.275|78|

The 24-hour seasonal naive baseline is strong on this short and regular series, and the lag-and-calendar linear baseline slightly improves RMSE. This result is useful because it shows that the local curve is predictable but too small to justify a deep representation-learning claim by itself. The public OPSD and UCI benchmarks form the reproducible main experiment layer, while this local curve is reserved for few-shot or local adaptation validation.
